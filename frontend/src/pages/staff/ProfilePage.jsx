...
import {
  ...
  LogOut, ToggleLeft, ToggleRight, Camera, UserCog,
  Store, Building2, ShieldCheck, ClipboardList, QrCode,
} from 'lucide-react';
...
const TABS = [
  { id: 'profile',  label: 'Personal Info', icon: User,     color: '#0ea5e9' },
  { id: 'work',     label: 'Work Info',     icon: Store,    color: '#6366f1' },
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div><h2 className="section-title">Personal Info</h2><p className="section-desc">Your personal details</p></div>
      ...
      <div className="card flex flex-col sm:flex-row items-center gap-5">
        <div className="relative shrink-0">
          {user?.avatar
            ...
            : <div className="w-20 h-20 rounded-2xl flex items-center justify-center text-2xl font-bold text-white" style={{ background: 'linear-gradient(135deg,#0ea5e9,#6366f1)' }}>{user?.name?.[0]?.toUpperCase() || '?'}</div>
          }
          {editing && <div className="absolute -bottom-2 -right-2 w-6 h-6 bg-sky-500 rounded-lg flex items-center justify-center"><Camera size={11} className="text-white" /></div>}
        </div>
        <div className="flex-1 text-center sm:text-left">
          <h3 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>{user?.name}</h3>
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{user?.email}</p>
          <div className="flex flex-wrap justify-center sm:justify-start gap-2 mt-2">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold" style={{ background: 'rgba(14,165,233,0.12)', color: '#38bdf8', border: '1px solid rgba(14,165,233,0.2)' }}>
              <UserCog size={11} /> Staff Member
            </span>
          </div>
        </div>
        <div className="text-center shrink-0">
          ...
          <p className="font-bold text-sm" style={{ color: 'var(--text-primary)' }}>
            {typeof user?.canteen_id === 'object' ? user?.canteen_id?.name : user?.canteen_id ? 'Canteen' : '—'}
          </p>
        </div>
      </div>
      ...
      {editing ? (
        ...
        <div className="card grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            ...
            { icon: UserCog, label: 'Role',      value: 'Staff Member' },
          ].map(({ icon: Icon, label, value }) => (
            <div key={label} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'var(--bg-elevated)' }}>
              <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: 'rgba(14,165,233,0.1)' }}><Icon size={14} style={{ color: '#0ea5e9' }} /></div>
...
const WorkInfoSection = ({ user }) => {
  ...
  const canteenName = canteen?.name || (user?.canteen_id ? 'Your Canteen' : null);

  return (
    <div className="space-y-5">
      <div><h2 className="section-title">Work Info</h2><p className="section-desc">Your canteen assignment and responsibilities</p></div>

      {!canteenName ? (
        <div className="card text-center py-12">
          <Store size={40} className="mx-auto mb-3 opacity-20" style={{ color: 'var(--text-muted)' }} />
          <p className="font-semibold" style={{ color: 'var(--text-primary)' }}>No canteen assigned</p>
          <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Contact your canteen owner to get assigned</p>
        </div>
      ) : (
        <>
          <div className="card flex items-center gap-5">
            {canteen?.image
              ? <img src={canteen.image} alt={canteenName} className="w-16 h-16 rounded-2xl object-cover shrink-0 border-2" style={{ borderColor: 'var(--border-color)' }} />
              : <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-2xl shrink-0" style={{ background: 'rgba(99,102,241,0.1)' }}>🍽️</div>
            }
            <div className="flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>{canteenName}</h3>
                <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold ${canteen?.is_open ? 'text-emerald-400' : 'text-red-400'}`}
                  style={{ background: canteen?.is_open ? 'rgba(16,185,129,0.12)' : 'rgba(239,68,68,0.12)' }}>
                  {canteen?.is_open ? '🟢 Open' : '🔴 Closed'}
                </span>
              </div>
              {canteen?.description && <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>{canteen.description}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              ...
              { icon: Phone,        label: 'Canteen Phone', value: canteen?.phone || 'Not set', color: '#0ea5e9' },
              { icon: ShieldCheck,  label: 'My Role', value: 'Staff Member', color: '#10b981' },
              { icon: Store,        label: 'Status', value: canteen?.is_open ? 'Operational' : 'Closed', color: canteen?.is_open ? '#10b981' : '#ef4444' },
            ].map(({ icon: Icon, label, value, color }) => (
              <div key={label} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'var(--bg-elevated)' }}>
                <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: `${color}15` }}><Icon size={14} style={{ color }} /></div>
                <div className="min-w-0">
                  <p className="text-2xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>{label}</p>
                  <p className="text-sm font-medium truncate" style={{ color: 'var(--text-primary)' }}>{value}</p>
                </div>
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button onClick={() => navigate('/staff/orders')} className="flex items-center gap-3 px-4 py-3 rounded-xl transition-all hover:opacity-90" style={{ background: 'linear-gradient(135deg,#6366f1,#818cf8)', color: '#fff' }}>
                <ClipboardList size={18} /><div className="text-left"><p className="font-bold text-sm">Live Orders</p><p className="text-xs opacity-80">View & manage orders</p></div>
              </button>
              <button onClick={() => navigate('/staff/qr-verify')} className="flex items-center gap-3 px-4 py-3 rounded-xl transition-all hover:opacity-90" style={{ background: 'linear-gradient(135deg,#0ea5e9,#38bdf8)', color: '#fff' }}>
                <QrCode size={18} /><div className="text-left"><p className="font-bold text-sm">QR Verify</p><p className="text-xs opacity-80">Scan pickup QR codes</p></div>
              </button>
            </div>
          </div>
        </>
    ...
    <div className="space-y-5">
      ...
      <div className="card">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={toggleTheme}>{isDark ? <ToggleRight size={28} style={{ color: '#0ea5e9' }} /> : <ToggleLeft size={28} style={{ color: 'var(--text-muted)' }} />}</button>
        </div>
      </div>
      ...
      <div className="card space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'rgba(14,165,233,0.1)' }}><Lock size={16} style={{ color: '#0ea5e9' }} /></div>
...
const StaffProfilePage = () => {
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-6"><h1 className="page-title">My Profile</h1><p className="page-subtitle">Personal profile and work information</p></div>
      <div className="flex gap-6 items-start">
        <aside className="hidden lg:flex flex-col w-52 shrink-0 sticky top-24 card p-2 gap-0.5">
          <div className="flex items-center gap-3 px-3 py-3 mb-2 rounded-xl" style={{ background: 'var(--bg-elevated)' }}>
            {user?.avatar ? <img src={user.avatar} className="w-9 h-9 rounded-xl object-cover" alt="" /> : <div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold text-white" style={{ background: 'linear-gradient(135deg,#0ea5e9,#6366f1)' }}>{user?.name?.[0]?.toUpperCase() || '?'}</div>}
            <div className="min-w-0"><p className="text-sm font-bold truncate" style={{ color: 'var(--text-primary)' }}>{user?.name}</p><p className="text-xs" style={{ color: 'var(--text-muted)' }}>Staff</p></div>
          </div>
        ...
        <div className="flex-1 min-w-0 space-y-5">
          ...
          {activeTab === 'work'     && <WorkInfoSection user={user} />}
